...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!slides || slides.length === 0) return NextResponse.json({ error: 'sem slides' }, { status: 400 });
  ...
  await uploadJson(resultPath, {
    jobId, status: 'queued', progress: 0, message: 'queued by /admin', updatedAt: new Date().toISOString()
  });
  ...
  return NextResponse.json({ jobId, manifestUrl: publicUrl(manifestPath) });
}
